...
async function fetchFromLibrary(supabase: any, mealType: string, target: MealTarget, count: number): Promise<MealAlternative[] | null> {
  console.log(`[fetchFromLibrary] CALLED — mealType="${mealType}" target=${target.calories}kcal want=${count}`)

  try {
    // AI generation only draws from standard (seeded) recipes — never a
    // coach's own or org-mates' recipes. Those are picked manually by the
    // coach via the recipe picker in the meal-plan editor instead.
    const { count: totalCount } = await supabase
      .from('recipes')
      .select('id', { count: 'exact', head: true })
      .eq('is_standard', true)
    console.log(`[fetchFromLibrary] "${mealType}": ${totalCount ?? '?'} standard recipes in DB`)
...
export async function POST(req: Request) {
  ...
  // AI generation only draws from standard (seeded) recipes, not org/coach recipes
  // (those are picked manually via the recipe picker in the meal-plan editor).
  ...
  for (let mi = 0; mi < mealTargets.length; mi++) {
    const lib = await fetchFromLibrary(supabase, mealTargets[mi].name, mealTargets[mi], alternatives_per_meal)
    if (lib) {
