      ...
      <section className="border border-gray-100 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-[#f9fefb]">
          <h2 className="font-semibold text-gray-900 text-sm flex items-center gap-2">
            ...
            {t('clientDetail.checkins.weeklyOverview')}
          </h2>
          <span className="text-xs text-gray-400">{weekly.length} {weekly.length === 1 ? t('clientDetail.checkins.checkinSingular') : t('clientDetail.checkins.checkinPlural')}</span>
        </div>
        {weekly.length === 0 ? (
          <div className="p-8 text-center">
            <ClipboardList className="w-8 h-8 text-gray-200 mx-auto mb-2" />
            <p className="text-sm text-gray-400">{t('clientDetail.checkins.noWeeklyYet')}</p>
          </div>
        ) : (
      ...
      <section className="border border-gray-100 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900 text-sm flex items-center gap-2">
            ...
            {t('clientDetail.checkins.dailyCheckins')}
          </h2>
          <span className="text-xs text-gray-400">{daily.length} {daily.length === 1 ? t('clientDetail.checkins.checkinSingular') : t('clientDetail.checkins.checkinPlural')}</span>
        </div>
        {daily.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-sm text-gray-400">{t('clientDetail.checkins.noDailyYet')}</p>
          </div>
        ) : (
