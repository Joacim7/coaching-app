...
const STATUS_CONFIG: Record<ClientStatus, { labelKey: TranslationKey; pill: string; dot: string }> = {
  active:     { labelKey: 'clients.status.active',     pill: 'bg-[#ebf5ef] text-[#1a5c3a]', dot: 'bg-[#2d8653]' },
  inactive:   { labelKey: 'clients.status.inactive',   pill: 'bg-gray-100 text-gray-500',    dot: 'bg-gray-400' },
  new:        { labelKey: 'clients.status.new',        pill: 'bg-[#ebf5ef] text-[#1a5c3a]', dot: 'bg-[#2d8653]' },
  onboarding: { labelKey: 'clients.status.onboarding', pill: 'bg-yellow-50 text-yellow-700', dot: 'bg-yellow-500' },
  course:     { labelKey: 'clients.status.course',     pill: 'bg-[#ebf5ef] text-[#1a5c3a]', dot: 'bg-[#2d8653]' },
  followup:   { labelKey: 'clients.status.followup',   pill: 'bg-orange-50 text-orange-700', dot: 'bg-orange-400' },
  app_access: { labelKey: 'clients.status.appAccess',  pill: 'bg-blue-50 text-blue-700',     dot: 'bg-blue-500' },
}

// ── Filter tabs ──────────────────────────────────────────────────────────────

const FILTERS = [
  { key: 'alle',       labelKey: 'clients.filter.all',       match: (_: ClientRow) => true },
  { key: 'aktive',     labelKey: 'clients.filter.active',    match: (c: ClientRow) => c.status === 'active' },
  { key: 'inaktive',   labelKey: 'clients.filter.inactive',  match: (c: ClientRow) => c.status === 'inactive' },
  { key: 'app',        labelKey: 'clients.status.appAccess', match: (c: ClientRow) => c.status === 'app_access' },
  { key: 'nye',        labelKey: 'clients.filter.new',       match: (c: ClientRow) => c.status === 'new' },
  { key: 'onboarding', labelKey: 'clients.status.onboarding', match: (c: ClientRow) => c.status === 'onboarding' },
  { key: 'kurs',       labelKey: 'clients.status.course',    match: (c: ClientRow) => c.status === 'course' },
  { key: 'oppfolging', labelKey: 'clients.status.followup',  match: (c: ClientRow) => c.status === 'followup' },
] as const satisfies readonly { key: string; labelKey: TranslationKey; match: (c: ClientRow) => boolean }[]
...
function relativeDate(iso: string, t: (key: TranslationKey) => string) {
  const d = Math.floor((Date.now() - new Date(iso).getTime()) / 86_400_000)
  if (d === 0) return t('clients.relative.today')
  if (d === 1) return t('clients.relative.yesterday')
  if (d < 7) return `${d}${t('clients.relative.daysAgoSuffix')}`
  if (d < 30) return `${Math.floor(d / 7)}${t('clients.relative.weeksAgoSuffix')}`
  if (d < 365) return `${Math.floor(d / 30)}${t('clients.relative.monthsAgoSuffix')}`
  return `${Math.floor(d / 365)}${t('clients.relative.yearsAgoSuffix')}`
}
...
export function ClientList({ clients: initial }: { clients: ClientRow[]; coachId: string }) {
  ...
  async function updateStatus(profileId: string, newStatus: ClientStatus) {
    ...
    if (!res.ok) {
      ...
      setStatusError(err.error ?? t('clients.updateStatusError'))
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <div className="relative">
          <select
            ...
            defaultValue="all"
          >
            <option value="all">{t('clients.allCoaches')}</option>
          </select>
      ...
      {displayed.length > 0 && (
        <div className="grid grid-cols-[1fr_150px_90px_90px_100px_36px] gap-3 px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">
          <span>{t('clients.columns.client')}</span>
          <span>{t('clients.columns.status')}</span>
          <span className="text-center">{t('clients.columns.mealPlan')}</span>
          <span className="text-center">{t('sidebar.training')}</span>
          <span className="text-center">{t('clients.columns.activity')}</span>
          <span />
        </div>
      )}
      ...
      {displayed.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="font-medium text-gray-500">{t('clients.noResults.title')}</p>
          <p className="text-sm mt-1">{t('clients.noResults.subtitle')}</p>
        </div>
      ) : (
              ...
              <div key={client.id} className="relative">
                ...
                >
                  ...
                  <div className="text-center">
                    {client.lastActivity ? (
                      <span className="text-xs text-gray-600">{relativeDate(client.lastActivity, t)}</span>
                    ) : (
                  ...
                  >
                    <button
                      ...
                      title={t('clients.changeStatusTitle')}
                    >
